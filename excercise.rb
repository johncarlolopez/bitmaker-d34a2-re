def check_syntax(str)
    ...
    if opening_chars.include?(char) # opening bracket
      ...
      if char == required_char # it's the right kind of closing bracket
        ...
        if open.any? # it's the wrong kind of closing bracket
          puts "* You have a syntax error: there is an unexpected #{char} where there should be #{required_char}"
  ...
  if open.any?
    required_characters = ''
    open.each{|char| required_characters += ' and ' + openables[char] }
    required_characters = required_characters.split(' ')[1..-1].join(' ')
    puts "* You have a syntax error: the string ended without a closing #{required_characters}"
  end
  return open.empty?
end
